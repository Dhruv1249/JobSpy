"""
Crypto Jobs List job board scraper.
"""
...
log = create_logger("cryptojobs")

class CryptoJobs(Scraper):
    """
    Scraper for Crypto Jobs List.
    """

    def __init__(self, proxies: list[str] | str | None = None, ca_cert: str | None = None, user_agent: str | None = None):
        """
        Initialize Crypto Jobs List scraper.
        """
        super().__init__(Site.CRYPTO_JOBS, proxies=proxies, ca_cert=ca_cert, user_agent=user_agent)
    ...
    def scrape(self, scraper_input: ScraperInput) -> JobResponse:
        """
        Scrape job postings from Crypto Jobs List.
        """
        ...
        for item in channel.findall("item"):
            ...
            if pub_date_raw is not None and pub_date_raw.text:
                try:
                    ...
                    log.debug(f"[cryptojobs] Skipping item due to error: {type(scrape_error).__name__}: {scrape_error}")
