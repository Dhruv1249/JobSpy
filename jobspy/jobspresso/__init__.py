...
class Jobspresso(Scraper):
    """
    Scraper for Jobspresso.
    """

    def __init__(self, proxies: list[str] | str | None = None, ca_cert: str | None = None, user_agent: str | None = None):
        """
        Initialize Jobspresso scraper.
        """
        super().__init__(Site.JOBSPRESSO, proxies=proxies, ca_cert=ca_cert, user_agent=user_agent)
    ...
    def scrape(self, scraper_input: ScraperInput) -> JobResponse:
        """
        Scrape job postings from Jobspresso.
        """
        ...
        for item in channel.findall("item"):
            ...
            if " @ " in title_text:
                position, _, rest = title_text.partition(" @ ")
                position = position.strip()
                if " (" in rest:
                    company, _, loc_part = rest.partition(" (")
                    company = company.strip()
                    loc_str = loc_part.rstrip(")")
                else:
                    company = rest.strip()
                    loc_str = "Worldwide"
            else:
