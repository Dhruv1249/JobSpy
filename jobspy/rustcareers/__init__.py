"""
Rust Careers job board scraper.
"""
...
log = create_logger("rustcareers")

class RustCareers(Scraper):
    """
    Scraper for Rust Careers.
    """

    def __init__(self, proxies: list[str] | str | None = None, ca_cert: str | None = None, user_agent: str | None = None):
        """
        Initialize Rust Careers scraper.
        """
        super().__init__(Site.RUST_CAREERS, proxies=proxies, ca_cert=ca_cert, user_agent=user_agent)
    ...
    def scrape(self, scraper_input: ScraperInput) -> JobResponse:
        """
        Scrape job postings from Rust Careers.
        """
        url = "https://rust.careers/feed"
        try:
        ...
        for item in channel.findall("item"):
            ...
            if " is hiring a " in title_text:
                company, _, position = title_text.partition(" is hiring a ")
            ...
            if pub_date_raw is not None and pub_date_raw.text:
                try:
                    ...
                    log.debug(f"[rustcareers] Skipping item due to error: {type(scrape_error).__name__}: {scrape_error}")
